...
| --- | --- | --- |
| `BloogBot/Game/ObjectManager.cs`, `MainViewModel.cs` (and the same edits in the .NET Framework tree) | `EnumerateVisibleObjects` clears `Player`, `Pet`, `playerGuid` and `Objects` when not logged in; the command-handler loop also tests `ObjectManager.IsLoggedIn` | the method only ever *updated* state while logged in, so on logout those kept pointing at memory the client had freed, for good. The command-handler loop reads `Player.Name` from a thread pool thread every 250 ms and hit one, killing the process with `Fatal error. 0xC0000005`. **This is a real .NET Framework to .NET 9 behavioural difference:** `BloogBot/App.config` sets `legacyCorruptedStateExceptionsPolicy`, so on .NET Framework the access violation arrived as a catchable `AccessViolationException` and the loop logged it and carried on - the bug was always there, just hidden. .NET 9 has no such setting and a corrupted-state exception is always fatal, so the dangling references had to go. `IsLoggedIn` only reads a fixed static address, so it is safe off the main thread. Applied to BOTH trees at the user's request |
| `BloogBot/AI/SharedStates/GrindState.cs` (and the same edit in the .NET Framework tree) | both uses of `container.GetCurrentHotspot()` - `HandleWpSelection` in the fork branch and the `#else` branch of `Update` - null-check it and call a new `WarnNoHotspot`, which logs once | a database with no matching hotspot made every tick throw `NullReferenceException`. The fork looks up hard-coded ids by map and faction, which only the `Sql` seed scripts create, so an unseeded database never resolves one. The message differs per branch because `IsAlly` is a fork addition to `LocalPlayer` and will not compile under `UseCustomChanges=false`, where `GetCurrentHotspot` returns `GrindingHotspot` anyway. Applied to BOTH trees at the user's request |
| `BloogBot/Navigation.cs` (and the same edit in the .NET Framework tree) | the static constructor checks once for an `mmaps` folder beside the assembly and logs if it is absent; `CalculatePath` then returns an empty path instead of calling into `Navigation.dll` | the native code builds the movemap path from its own module directory and does not check it exists, so pathfinding without the tiles faulted inside native code and surfaced as `SEHException: External component has thrown an exception` once per tick, naming nothing useful. An empty path feeds the `path.Length <= 1` branch `GetNextWaypoint` already has, so the bot falls back to heading straight for its destination - the behaviour that branch was written for. `Directory.Exists` was checked to resolve through a directory junction, which is how the tiles are usually linked in. Applied to BOTH trees at the user's request |
