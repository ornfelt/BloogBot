...
High-water mark: `ade1551` (every commit up to and including this one is decided)
...
Remaining after the high-water mark: `54`
...
None. The next run starts at `a3733da` ('MoveToPositionState: allow setting a deadline') - note that one *does* touch a guarded file, so expect a real conflict and a step 6 mirror decision.
